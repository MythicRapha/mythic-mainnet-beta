const { Connection, Keypair, PublicKey, SystemProgram, TransactionInstruction, Transaction, sendAndConfirmTransaction } = require('/mnt/data/mythic-cli/node_modules/@solana/web3.js');
...
async function initMythToken() {
  ...
  const data = Buffer.concat([
    ...
    serializeBorsh([
      ['pubkey', FOUNDATION],
      // gas_split: 70% validator, 20% foundation, 10% burn
      ['u16', 7000], ['u16', 2000], ['u16', 1000],
      // compute_split: 60% validator, 25% foundation, 15% burn
      ['u16', 6000], ['u16', 2500], ['u16', 1500],
      // inference_split: 50% validator, 30% foundation, 20% burn
      ['u16', 5000], ['u16', 3000], ['u16', 2000],
      // bridge_split: 40% validator, 40% foundation, 20% burn
...
async function main() {
  ...
  const ok = results.filter(function(r) { return r; }).length;
  const fail = results.filter(function(r) { return !r; }).length;
